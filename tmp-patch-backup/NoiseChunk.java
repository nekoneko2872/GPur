...
public class NoiseChunk implements DensityFunction.ContextProvider, DensityFunction.FunctionContext {
    ...
    private int gpurInterpolationCacheCellX = Integer.MIN_VALUE;
    private int gpurInterpolationCacheCellZ = Integer.MIN_VALUE;
    private @Nullable float[] gpurInterpolationCache;
    ...
    private void invalidateGpurInterpolationCache() {
        ...
        this.gpurInterpolationCacheCellZ = Integer.MIN_VALUE;
    }

    private void prepareGpurInterpolationCache(int z) {
        ...
        if (this.gpurInterpolationCache != null
            ...
            && this.gpurInterpolationCacheCellZ == z
            && this.validateGpurInterpolationCache(this.gpurInterpolationCache)) {
            return;
        }

        final int interpolatorCount = this.interpolators.size();
        final float[] packedCorners = new float[interpolatorCount * this.cellCountY * 8];
        int cornerIndex = 0;
        for (NoiseChunk.NoiseInterpolator noiseInterpolator : this.interpolators) {
            for (int cellY = 0; cellY < this.cellCountY; cellY++) {
                packedCorners[cornerIndex++] = (float)noiseInterpolator.slice0[z][cellY];
                packedCorners[cornerIndex++] = (float)noiseInterpolator.slice1[z][cellY];
                packedCorners[cornerIndex++] = (float)noiseInterpolator.slice0[z][cellY + 1];
                packedCorners[cornerIndex++] = (float)noiseInterpolator.slice1[z][cellY + 1];
                packedCorners[cornerIndex++] = (float)noiseInterpolator.slice0[z + 1][cellY];
                packedCorners[cornerIndex++] = (float)noiseInterpolator.slice1[z + 1][cellY];
                packedCorners[cornerIndex++] = (float)noiseInterpolator.slice0[z + 1][cellY + 1];
                packedCorners[cornerIndex++] = (float)noiseInterpolator.slice1[z + 1][cellY + 1];
            }
        }

        final float[] values = GPurServices.generation()
            .interpolateNoiseColumn(this.cellWidth, this.cellHeight, this.cellCountY, interpolatorCount, packedCorners);
        if (!this.validateGpurInterpolationCache(values)) {
            this.invalidateGpurInterpolationCache();
            return;
        }
    ...
    private boolean validateGpurInterpolationCache(final @Nullable float[] values) {
        ...
        final int samplesPerInterpolator = this.cellCountY * this.cellHeight * this.cellWidth * this.cellWidth;
    ...
    private int gpurInterpolationIndex(final int interpolatorIndex) {
        ...
        final int samplesPerInterpolator = this.cellCountY * samplesPerCell;
    ...
    public void selectCellYZ(int y, int z) {
        ...
        this.prepareGpurInterpolationCache(z);
    ...
    public class NoiseInterpolator implements DensityFunctions.MarkerOrMarked, NoiseChunk.NoiseChunkDensityFunction {
        ...
        public double compute(DensityFunction.FunctionContext context) {
            if (context != NoiseChunk.this) {
                ...
                if (NoiseChunk.this.fillingCell && NoiseChunk.this.gpurInterpolationCache != null) {
